...
export function registerValidationTools(server: McpServer, config: Config): void {
  server.registerTool(
    "validate_code",
    {
      ...
      description:
        "Run checks and hints, not catalogs (`list_catalog`) and not edit plans (`plan_change`); prefer `analyze_plugin` for a descriptive report and `trace_symbol` for dependency graphs — this tool only scores/lints/hints, it never describes structure.\n`mode=plugin` scores a plugin against official pitfalls (target required); `grep` scans the snapshot for TODOs/console.log/undefined/lang/hardcoded HTML (not a shell, no code execution); `i18n` looks up `key` or, if omitted, coverage vs en.js; `build` returns the npm/gulp command for a goal as text — it never runs it.\nExample: `mode=grep` `checks=['todos','console_logs']`; `mode=i18n` `key='player_playlist'` or omit `key` for a full coverage table.\n`checks` only applies to mode=grep (default: all five checks); `show_missing` only applies to i18n coverage and is ignored once `key` is set; `goal` only applies to mode=build; `target` is required only for mode=plugin (folder or JS path) — missing plugin → error listing folders, missing `target` for mode=plugin → error naming the requirement.",
      inputSchema: {
        mode: z
